...
export default function HowItWorks() {
  ...
  return (
    <section className="section-tinted py-28 px-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className={`reveal-stagger ${visible ? "is-visible" : ""} mt-14 grid gap-5 md:grid-cols-3`}>
          {steps.map((step) => (
            <div
              key={step.number}
              className="border border-neutral-800/40 bg-neutral-950 p-7"
            >
              <div className="flex items-center gap-4 mb-5">
                <span className="text-neutral-600">{step.icon}</span>
                <span className="text-neutral-700 font-mono text-[10px] tracking-[0.2em]">
                  {step.number}
                </span>
              </div>
